...
| **E20 (v3.1, post-freeze PF-1 → PF-3; RAN 2026-09-17, report-back `spec/e20_reportback.md`)** | **18d_e20_refugia_audit → 18e_e20_solves (PF-1) / 18f_e20_consistent_solves (PF-2) → 18g_e20_analysis** | y2y-geo / y2y-r / y2y-geo | value-shape conditionality: the zero-solve audit of every continuous value under its defensible shapes (+ the precision-relative contrast test), one guarded S0 run under saturating refugia (PF-1) and two under the consistent set (saturating refugia + convex transboundary and corridor connectivity, at t = 1 and with R2's connectivity target 0.198; PF-2), four block-split runs (PF-3: five blocks at 20% / corridors removed, each on S0 and S2), the transform-conditional share, own land and pinch-point pinning per arm (not in `run_v31.sh`) |
| **PF-4 corridors-out isolation (v3.1, spec v0.18.1; BUILT 2026-09-17, Ethan runs)** | **18h_pf4_corridors_out_arm → 18i_pf4_corridors_out_solve → 18j_pf4_corridors_out_analysis** | y2y-geo / y2y-r / y2y-geo | does climate corridors shape the registered balanced map? one certified anchor on S0@SSP585 at identity shapes with corridors at weight 0 and transboundary on the connectivity block's full 25% (weights re-derived, M4.37) → Jaccard on discretionary selections vs the registered anchor, swap composition, captures for the eight values + the EFG block, corridor capture as a co-benefit, the register's reading rule (near 1 → decoration, Decision 1 = B; below → a real change); pre-condition on Decision 1 (not in `run_v31.sh`) |
| **MANIFEST v4 freeze (spec v0.20; BUILT 2026-09-17, Ethan runs first)** | **11c_manifest_v4_freeze** | y2y-geo | the re-registered design: refugia velocity pre-checks + floor, transboundary current squared (+ dust rule), the self-contained v4 stack (`input_data/aligned_stack_v4/`), the re-audit cards (semantic gate withholds transboundary's target), five-block accounting at 20% and the seven scenarios' (w, t) pairs, manifest v4 frozen with its sha (14 voting cells). Then 12 → 13 → 15 → 18 → 18b → 18c → 19 → 20 → 21 under `VERSION v4` (unguarded MGA on the reference cell only; guarded band = the estimand; six E17-T3 arms; region-named clusters; the v3.1 clusters' survival) |
