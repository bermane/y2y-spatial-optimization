...
| **E20 (v3.1, post-freeze PF-1 + PF-2)** | **18d_e20_refugia_audit → 18e_e20_solves (PF-1) / 18f_e20_consistent_solves (PF-2) → 18g_e20_analysis** | y2y-geo / y2y-r / y2y-geo | value-shape conditionality: the zero-solve audit of every continuous value under its defensible shapes (+ the precision-relative contrast test), one guarded S0 run under saturating refugia (PF-1) and two under the consistent set (saturating refugia + convex transboundary and corridor connectivity, at t = 1 and with R2's connectivity target 0.198; PF-2), four block-split runs (PF-3: five blocks at 20% / corridors removed, each on S0 and S2), the transform-conditional share, own land and pinch-point pinning per arm (not in `run_v31.sh`) |
