...
**Status:** v0.12 — Gates 0a/0/1/2a/2b COMPLETE. S4 pilot PASS (tails 0.960/0.772 vs ≥0.75; places-through-pressure validated; zero formulation change). MGA verdict PLATEAU-RICH (D=0.953, C=0.020 at g=5%, rule v2_8db80fed1c702638) — Claim A carries. Tail-mask contingency RESCINDED (M4.14). **GATE 3 RATIFIED: freeze the manifest and pre-registration, then Gate 4 (ensemble + E1–E4, E7–E10).**
...
## Changelog
- v0.12.1 — E11 added (Ethan's design addition): two-level solution-space spread separating between-formulation divergence from within-formulation near-optimal freedom, in matched currency (pairwise Jaccard on discretionary selections; anchors vs within-cell diameters — envelope comparison only, never anchor-vs-mean, since MGA members are extremes by construction), PLUS the cross-objective suboptimality matrix Δ(s,s′) evaluating every anchor under every other cell's objective (zero solves; recomputation from representation CSVs). Central interpretive question made measurable: does moving between value positions change the map more or less than the tolerance already inside one position? Anchors mutually inside each other's g-bands = certified no-regrets form of value pluralism; large-Δ pairs = measured value conflict, localized by E3. F10 added; runs at Gate 4 alongside E1–E4.
...
| E11 (NEW v0.12.1) | Two-level solution-space spread: between-anchor distance matrix vs within-cell diameters (matched currency) + cross-objective suboptimality Δ(s,s′) = (obj_{s′}(x_s) − obj*_{s′})/obj*_{s′} | Separates value disagreement from near-optimal freedom | Zero solves (anchor recomputation from representation CSVs). Guardrail: MGA members are extremes — compare anchor distances to within-cell MAXIMA (envelope question), never to means. Readings: anchors inside each other's g-bands ⇒ value pluralism absorbable within tolerance (certified no-regrets form); large Δ pairs ⇒ measured value conflict, localized by E3. F10 |
...
**4 — Ensemble** (14 cells serial, ~45–60 min/cell ⇒ ~10–14 h) **+ E1–E4, E7–E10.**
...
All resolved or deferred. **v0.12:** S4 pilot PASS ratified; tail-mask contingency RESCINDED (M4.14 — no standing escalation authorization; future failures return to design discussion); Gate 3 authorized with freeze confirmations (climate-cell mechanics; E4 reference-cell-only). **v0.11:** carbon-forward places-mechanism = (w, t) on the existing stack. **v0.10:** D-A(a) MGA estimator + re-registered estimand; verdict rule v2 (now measured: PLATEAU-RICH); k-best retained one-per-cell. D-B amount-vs-places as scenario axis: S0 amount (unchanged). **Rulings:** no iterate-once on the m_soc realized-share miss; ensemble LP twins on Gurobi LP path with HiGHS spot-check ×2. Standing: D3(b); EFGs locked; doubling-only; constants frozen; mass-proportional carbon split. Deferred: multi-budget; Marxan E6.
