...
what they are, code in parentheses.** Alberta re-run = stage 2 (its v3 stack exists; spec mirror re-pin pending). **REPORT-BACK for the v3.1 run + package: `spec/v31_reportback.md` (2026-09-15; R10.22 measured) → chat.** **SPEC v0.18 = FROZEN; post-freeze register PF-1 = E20 refugia transform conditionality (M4.34, R10.24): `18d_e20_refugia_audit` (py, RAN: floor inert, log1p(1/v) kills the θ-tail, ordering unchanged) → `18e_e20_solves` (R, Ethan ~1 h: S0@585 guarded under log1p(1/v) → `runs_v3.1/e20_log/`) → `18g_e20_analysis` (py: transform-conditional share per arm). **PF-2 (2026-09-16, M4.35, R10.25): the uniform value-shape audit in 18d (every continuous value × {linear, saturating, convex}; I² transboundary = concentrated-satiating → R2 target 0.198; Carroll layer = compressed index, I² not adopted; AOH log1p inexpressible; gHM convex tail only below precision) → `18f_e20_consistent_solves` (R, Ethan ~1 h: S0@585 under log1p(1/v) + I² connectivity at t 0.198 → `runs_v3.1/e20_consistent/`) → 18g reads every solved arm. **PF-1 RESULT (R10.24 cont.): transform-conditional share 4.3% — the core rests on the refugia ORDERING, not the 1/v tail; RULING: 1/v stays registered, log1p in the pocket. PF-2 arms (18f: t = 1 and R2-target 0.198; corridors squared too, M4.35 add. 4) pending Ethan's run. **PF-3 BUILT (spec v0.18 sequence, M4.36): 18d writes four block-split arms (A: five blocks at 20%; B: corridors removed) × (S0, S2) on the step-2 shapes; 18f runs all six arms (PF-3 gated on the step-2 baseline `e20_consistent`); 18g reports own land (E18 definition), pinch-point pinning, D, Jaccards vs the registered run and the baseline (`spec/v3.1/E20_pf3_summary.csv`). Weights per arm in `e20_refugia_transform.json`. **PF-2/PF-3 RAN 2026-09-17 (18f Ethan, 18g headless; R10.25 cont., M4.36 addendum; report-back `spec/e20_reportback.md` → chat):** consistent shapes at t = 1 → conditional share 5.0%, core survival 87%, the raw-current spike PINNED 100% (registered S0 22.9%) but own land 4 km² (the spike is S2's registered land); R2's target 0.198 UN-PINS it (7.0%, M6.7 again); block splits: transboundary at per-layer share 0.20–0.25 owns ~12.8k km² at the balanced S0 (crossover moved from 0.33–0.42 at identity to (0.125, 0.20] under I²), corridors add nothing (A vs B tiers Jaccard 0.846), core survival 63–65% at S0 / 23–35% at S2 with the lost land 86–94% densest refugia, D 0.66–0.81. 18g now carries the registered S0/S2 reference rows. Nothing registered changed. **SPEC v0.18.1 (2026-09-17): E20 ratified as evidence (core SHAPE-ROBUST; D-PF1 frozen shapes stand for paper 1, connectivity value model reopened for the applied paper under an R2 semantic gate; D-PF2 variant A OFF the table, live choice = B vs the registered 50/50 block, HELD on PF-4; D-PF3 deck caveat rewritten). PF-4 BUILT (M4.37): `18h_pf4_corridors_out_arm` (py: writes the arm → `spec/v3.1/pf4_corridors_out.json`) → `18i_pf4_corridors_out_solve` (R: ONE certified anchor, corridors w = 0, transboundary on the full 0.25, ~1 min) → `18j_pf4_corridors_out_analysis` (py: Jaccard on discretionary selections, swap composition, captures for 8 values + EFG block, corridor co-benefit, the register's reading rule with 'near 1' operationalized at 0.95, DISCLOSED). Numeric order = run order (Ethan's rule). ETHAN RUNS IT — a headless first pass was discarded at his instruction (build, never run, even when the request says 'run'); **PF-4 RAN (Ethan, 2026-09-17; R10.26): Jaccard 0.686 on discretionary selections → reading rule 'noticeably below' → corridors WAS shaping the balanced map, B is a real change; but the swap is OFF-CORE (core 29,088 → 28,782 of 29,194 km²), captures move ≤3.2 pts, corridors keeps 90% of its discretionary capture at weight 0 — places not outcomes; Decision 1 → chat (report-back addendum in `spec/e20_reportback.md`).**** Outside run_v31.sh; director outputs untouched.**
**MANIFEST v4 = THE REGISTERED DESIGN (study plan v0.20, Ethan 2026-09-17; supersedes v3.1, which stays as the disclosed prior registration; BUILT the same day, pre-registered before any solve; M2.12 / M4.38 / M4.39; R10.27 + R11.1–R11.3 placeholders).** Trigger = E20 + PF-4 (the two-layer connectivity block halved each layer's expressivity; pinch points pin only under a convex shape with NO target; 1/v's tail top partly a v→0 artifact). Design: FIVE discretionary blocks at 20% (`config.BLOCKS_FIVE` via `blocks_for("v4")`: core habitat = macrorefugia 1/v WITH a velocity floor (one cell width / 110 yr = 0.00909 km/yr, per realization, expected inert); structural connectivity = transboundary current SQUARED, t = 1, NO target (R2 SEMANTIC GATE, now registered); climate corridors identity; carbon m_soc θ-target + biomass 74.2/25.8; biodiversity equal); EFG block + naturalness unchanged; SEVEN scenarios (S2c climate-corridors-forward NEW; doubling rule 0.40 / 0.15) × 2 climates = 14 voting cells; constants unchanged. **ONE SWITCH `config.Y2Y_VERSION = "v4"` (default):** `Y2Y_STACK_DIR` = `input_data/aligned_stack_v4/` (a self-contained copy: unchanged layers sha-identical, the two re-shaped layers, its own manifest.json — FLAGSHIP-scoped; `HANDOFF_DIR` = `aligned_stack/` stays canonical for 02 / the northern audit / everything else), `runs_v4/`, `spec/manifest_v4.csv` (+ sha; new columns block_structure, value_shapes, macrorefugia_path, reference_cell, unguarded_probes, e12_seed, floor_g, efg_block_version), `spec/v4/`, `scenarios_v4.json`; `efg_subdir_for` is a table (v4 → iucn_efg_v3); `director_core` tables version-scoped (BLOCK_AXES_FIVE → 7 star axes, VALUE_THEMES 6, ACT2_SCENARIOS s1/s2/s2c/s3/s4, MULTI_OWNER = 99, N_DESIGN 14). **UNGUARDED BAND = THE REFERENCE CELL ONLY** (spec step 1: s0_ssp585_theta5 at g = 2/5/10%); the GUARDED band is the estimand for all 14 → 13/15/18c compute F, E1, E3, E13, E19-T1 on guarded members (`BAND = "guard"`), E14 on the reference cell's unguarded members, E4 f(g) from the probes; 19 writes `f_unguarded_reference.tif` (no ensemble F_unguarded under v4). Build: **`11c_manifest_v4_freeze`** (py: velocity pre-checks + floor, I² + dust rule, the v4 stack, re-audit cards `audit/feature_cards_v4/`, scenarios v4, manifest v4 frozen) → **12** (`VERSION <- "v4"`: anchors + twins ×14, anchor written first, unguarded MGA on the reference cell only, refugia path per row) → 13, 15 → **18** (guarded ×14, floors on the manifest's five blocks, ~14 h) → **18b** (E19-T2 ×14; E17-T3 = SIX arms) → 18c → **19** (archives the v3.1 package to `director_package/_superseded_v3.1/`; region naming `dc.coexistence_layer/region_of` from `analyses/communities/spec/region_lookup.csv` (02_region_names; Ethan vets the words; default label = the region word, `dp.STYLE["cluster_region_labels"]`); T-D7 mean-vs-median check; the v3.1 clusters' survival cell) → 20 → 21. NOT re-run: k-best, s1x/s3x, E18/E20/PF-4 (v3.1 evidence), E12/E9/E10 (nb 16, after the workshop; 16/17 unswitched). NO Act 3 clusters (Ethan) — the coexistence layer is naming only. T-D4 ecoregions LANDED 2026-09-17 evening: `input_data/ecoregions/ecoregions2017_clipped.shp` (RESOLVE 2017, 25 ecoregions by `ECO_NAME`, 0.5% of PU cells outside every polygon at the clip edge) — 19 renders it on the next run. Pinned notebooks (18d–18j v3.1; 22/23 v1) need `Y2Y_VERSION=<that>` in the env. Regression: `Y2Y_VERSION=v3.1` reproduces the v3.1 records (verified 2026-09-17). Report-back for Ethan's run: `spec/v4_reportback.md` (to write after 19). **v0.17.3 (same day) → MANIFEST v3.1 = the one to solve:** the clip-edge flags were resolved by deriving the rarity-scaled targets from each
...
T-D7 consequences tables per act = cluster mean value ÷ allocatable-land mean, shown as "2.3×" (`ValueRatios`); deck is v3.1-only (M4.32).** **RUN = ETHAN, IN VS CODE, IN NUMERIC ORDER** (the one-command runner was deleted in 3e8b8b9; never headless-run a solve — build, Ethan runs; zero-solve smoke runs of Python notebooks are tolerated): 11c → 12 → 13 → 15 → 18 → 18b → 18c → [18b + 18c again iff the E19 gate opens] → 19 → 20 → 21. **EFG-BLOCK EXPOSURE (2026-09-08, R10.18): the five smallest classes (3–409 cells) are 100% inside the
...
**ONE VERSION SWITCH (M15.4): `config.ab_paths(version)` mirrors `y2y_paths()` on `config.Y2Y_VERSION`** — the flagship's default is now v4 (2026-09-17), so EVERY Alberta notebook runs with `Y2Y_VERSION=v3.1` in the environment until the mirror is re-pinned to v4 (config.BLOCKS is five blocks under v4 and the Alberta scenarios are four-block); v1 =
